...
def main():
    ...
    while True:
        Timer(CONFIG.TIMER_STD_VAL, obj.lr_calculate,[5,]).run()

        if CONFIG.trading_exchange == "NSE" or CONFIG.trading_exchange == "NFO":
            if (datetime.datetime.now().replace(microsecond=0) >= market_end_time and not CONFIG.SIMULATION_MODE):
                print("Exiting Linear forecast thread as NSE/NFO market has closed now ")
                return

        elif CONFIG.trading_exchange == "MCX":
            if (datetime.datetime.now().replace(microsecond=0) >= market_end_time and not CONFIG.SIMULATION_MODE):
                print("Exiting Linear forecast thread as MCX market has closed now ")
                return
